...
loop
{
  ...
  if ErrorLevel
  {
    ...
    loop
    {
      ...
      RegExReplace(newMail, "(*UCP)\w",, utfcount)
      RegExReplace(newMail, "\w",, ansicount)
      ...
      if (newMail != "") && (utfcount > 45)
        if (lastMail != newMail)
          if (utfcount > 45) && (ansicount <1)
          {
            ...
            mail := translate(newMail, "false")

            GuiControl, Text, Overlay, %mail%
            Loop {
              lastMail := dqx.readString(baseAddress + mailAddress, sizeBytes := 0, encoding := "utf-8", mailOffsets*)
              Sleep 250
            }
